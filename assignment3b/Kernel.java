...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		// hw3b: implemente the logic for case WAIT
	    case WAIT:
		...
		return waitQueue.enqueueAndSleep(myTid); // return a child thread id who woke me up
	    // hw3b: implemente the logic for case EXIT
		case EXIT:
		myTcb = scheduler.getMyTcb();
		// get the current thread's parent id
		int PtID = myTcb.getPid();				//parent thread id
		int CtID = myTcb.getTid();				//get the current thread's ID
		// search waitQueue for and wakes up the thread under the
		waitQueue.dequeueAndWakeup(PtID, CtID);
